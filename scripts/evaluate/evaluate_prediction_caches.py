...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="Tags corresponding to each prediction file.",
    )
    parser.add_argument(
        "--data",
        type=Path,
...
def evaluate_prediction_cache(
    *,
    ...
    validation_dir = output_dir / prediction_tag
    ...
    # per_class: dict[int, dict[str, Any]] (key -> class_id)
...
def main() -> int:
    ...
    prediction_tags = args.prediction_tags
    if len(prediction_tags) != len(args.predictions):
        raise ValueError(
            f"--prediction-tags supplied {len(prediction_tags)} values but "
            f"{len(args.predictions)} prediction files were provided."
        )
    ...
    for prediction_path in args.predictions:
        results.append(
            evaluate_prediction_cache(
                ...
                prediction_tag=prediction_tags[args.predictions.index(prediction_path)],
